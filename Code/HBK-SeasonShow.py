...
#sample_data = data_set.sample(20,random_state=1)

data_set = data_set.transpose()
df = pd.DataFrame()

# df['SBRY.L'] = data_set['SBRY.L']
# df['SVT.L'] = data_set['SVT.L']
df['MKS.L'] = data_set['MKS.L']
#
...
#
# cluster_model = KMeans(n_clusters=2).fit(df)
#
# for i in range(2):
#    single_cluster_visualization(df, cluster_model.labels_, i)

# for i in range (target_num):
#     single_cluster_visualization(sample_data,cluster_result,i)


# target_num = 8
# start_num = 2

# cluster_model = KMeans(n_clusters=target_num).fit(sample_data)
# [cluster_result,cluster_centroids,num_cluster] = hierarchical_based_Kmeans(sample_data,start_num,target_num)
# for i in range (target_num):
#     single_cluster_visualization(sample_data,cluster_result,i)

# for i in range(target_num):
#    single_cluster_visualization(sample_data, cluster_model.labels_, i)
